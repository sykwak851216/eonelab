...
// 				, addClasses : "big"
		...
		, _bindEvent = function(){
			...
			//[선택] 체크박스 클릭 시
			$("#inquiry-tray-list-grid").on("touchend click", "input[type=checkbox]", function() {
			...
			$(".btn-add").on("touchend click", function(e){
				e.preventDefault();
				location.href="<c:url value='/solutions/eone/pc/inquiry-tray-regist.tmpage' />"
			});

		}
		, _setForm = function(){
			$("#orderTypeCd").val('<%=EOrderType.INQUIRY.name()%>');
		...
		, list = function(paging){
			...
			SfpAjax.ajax("<c:url value='/solutions/eone/wmd/workplantray/getPagingList'/>", _param, function(data) {
				$("#inquiry-tray-list-grid").grid("draw", data.list, function(row){
					if(row.cancelYn === 'Y'){
