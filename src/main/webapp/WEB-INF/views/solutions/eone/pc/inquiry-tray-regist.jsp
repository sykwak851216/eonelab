...
<script>
$(function(){
	window.InquiryTrayRegist = (function($){
		var init = function(){
			$("#inquiry-tray-regist-grid").grid({
				header:[
					[
		...
		, _bindEvent = function(){
			...
			$(".grid-line-add").click(function() {
				var params = {}
				$('#inquiry-tray-regist-grid').grid("addRows", params, _convert);
		...
		, _getTray = function($tr){
			var param = $("#inquiry-tray-regist-grid").grid("getData", $tr)
			SfpAjax.ajax("<c:url value='/solutions/eone/wmd/traylocation/getTrayLocation'/>", param, function(data) {
				if (data) {
					//$("#output-tray-regist-grid").grid("draw", data, function(row){
		...
		, _registTray = function(){
			var $contents = $("#inquiry-tray-regist-grid");
			if($contents.validate()){
				...
				SfpAjax.ajax("<c:url value='/solutions/eone/wmd/workplantray/registerInquiryTrayList'/>", gridData, function(data) {
...
<main class="pop-view">
	<header>
		<h2><s:interpret word='호출업무 > Rack 등록' abbr='' /></h2>
	</header>
	...
	<div class="contents-wrap">
		<section>
			...
			<div id="inquiry-tray-regist-grid"></div>
		...
		<footer>
			<div class="align-right">
				...
				<label class="label-text" style="padding-right: 10px;"><s:interpret word='개의 Rack 호출' abbr='' /></label>
				  -->
...
<script type="text/sfp-template" data-model="inquiry-tray-regist-grid">
<tr>
