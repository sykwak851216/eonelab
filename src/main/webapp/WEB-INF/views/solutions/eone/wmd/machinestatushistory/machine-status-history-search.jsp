...
$(function(){
	...
	window.MachineStatusHistorySearch = (function($){
		var init = function(){
		...
		, _bindEvent = function(){
			...
			$("#machine-status-history-grid").on("click", ".btn-select", function() {
				var data = $("#machine-status-history-grid").grid("getData", this);
				Dialog.close(data);
			});
		...
		, list = function(paging){
			SfpAjax.ajax("<c:url value='/solutions/eone/wmd/machinestatushistory/getList'/>",
				$.extend($(".function-area").find("select, input").serializeObject(), { delYn : "N" })
				, function(data) {
					$("#machine-status-history-grid").grid("draw", data);
			});
		};
		init();
		return {
