...
$(function(){
	...
	window.SystemOperationModeAdd = (function($){
		var init = function(){
			_setForm();
			_bindEvent();
		}
		, _bindEvent = function(){
			...
			$(".btn-add").click(function() {
				_addSystemOperationMode();
			});
		}
		, _setForm = function(){

		}
		, _addSystemOperationMode = function(){
			var $contents = $(".contents-wrap");
			if($contents.validate()){
				SfpAjax.ajax("<c:url value='/solutions/eone/wmd/systemoperationmode/add'/>", $contents.find("select, input, textarea").serialize(), function(data) {
					alert("저장에 성공하였습니다.");
					Dialog.close(true);
				});
			}
...
<main>
	<header>
		<h2><s:interpret word='시스템 동작 모드 등록' abbr='' /></h2>
	</header>
