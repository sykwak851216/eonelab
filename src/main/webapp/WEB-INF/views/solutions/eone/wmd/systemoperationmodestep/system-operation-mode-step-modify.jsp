	...
	window.SystemOperationModeStepModify = (function($){
		...
		, _bindEvent = function(){
			...
			$(".btn-mod").click(function() {
				_modifySystemOperationModeStep();
			});
		}
		, _setForm = function(){
		...
		, _modifySystemOperationModeStep = function(){
			var $contents = $(".contents-wrap");
			if($contents.validate()){
				SfpAjax.ajax("<c:url value='/solutions/eone/wmd/systemoperationmodestep/modify'/>", $contents.find("select, input, textarea").serialize(), function(data) {
					alert(SfpUtils.getPhrase("msg-054"));
					Dialog.close(true);
				});
			}
		}
		, _deleteSystemOperationModeStep = function(){
			if(confirm(SfpUtils.getPhrase("msg-003")) == true){
				SfpAjax.ajax("<c:url value='/solutions/eone/wmd/systemoperationmodestep/delete'/>", $(".contents-wrap").find("select, input, textarea").serialize(), function(data) {
...
<main>
	<header>
		<h2><s:interpret word='시스템 동작 모드 단계 수정' abbr='' /></h2>
	</header>
