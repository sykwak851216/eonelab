...
$(function(){
	...
	window.RackCellSearch = (function($){
		var init = function(){
			$("#rack-cell-grid").grid({
				header:[
					[
		...
		, _bindEvent = function(){
			...
			$("#rack-cell-grid").on("click", ".btn-select", function() {
				var data = $("#rack-cell-grid").grid("getData", this);
				Dialog.close(data);
			});
		...
		, list = function(paging){
			SfpAjax.ajax("<c:url value='/solutions/eone/wmd/rackcell/getList'/>",
				$.extend($(".function-area").find("select, input").serializeObject(), { delYn : "N" })
				, function(data) {
					$("#rack-cell-grid").grid("draw", data);
			});
		};
		init();
		return {
