...
$(function(){
	...
	window.OrderWorkModify = (function($){
		...
		, _modifyOrderWork = function(){
			var $contents = $(".contents-wrap");
			if($contents.validate()){
				SfpAjax.ajax("<c:url value='/solutions/eone/wmd/orderwork/modify'/>", $contents.find("select, input, textarea").serialize(), function(data) {
					alert(SfpUtils.getPhrase("msg-054"));
					Dialog.close(true);
				});
			}
		}
		, _deleteOrderWork = function(){
			if(confirm(SfpUtils.getPhrase("msg-003")) == true){
				SfpAjax.ajax("<c:url value='/solutions/eone/wmd/orderwork/delete'/>", $(".contents-wrap").find("select, input, textarea").serialize(), function(data) {
...
<main>
	<header>
		<h2><s:interpret word='지시별 작업 내역 수정' abbr='' /></h2>
	</header>
	...
	<div class="contents-wrap">
		<section>
			<div class="register-table">
				...
				<ul>
					<li>
						<div class="title"><s:interpret word='작업번호' abbr='' /><em>*</em></div>
						<div class="cont">
							<div class="line-wrap">
								<span><input type="text" name="workId" placeholder="<s:interpret word='작업번호' abbr='' />" data-valid-max-size='30' data-required="true"  /></span>
							</div>
						</div>
					</li>
					<li>
