...
$(function(){
	...
	window.OrderTraySearch = (function($){
		var init = function(){
		...
		, _bindEvent = function(){
			...
			$("#order-tray-grid").on("click", ".btn-select", function() {
				var data = $("#order-tray-grid").grid("getData", this);
				Dialog.close(data);
			});
		...
		, list = function(paging){
			SfpAjax.ajax("<c:url value='/solutions/eone/wmd/ordertray/getList'/>",
				$.extend($(".function-area").find("select, input").serializeObject(), { delYn : "N" })
				, function(data) {
					$("#order-tray-grid").grid("draw", data);
			});
		};
		init();
		return {
